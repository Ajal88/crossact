...
def generateUserItemStackoverflow(i_user_file, i_user_answer_file, i_user_favorite_file, o_user_item_file, o_user_file,
                                  min_item):
    print('Loading user data... \n')
    user_item_count = {}
    user_answer_count = {}
...
def generateFMTrainingTestingWithNegSampleStackOverflowWithPartition(user_item_file, train_ratio, n_Neg, n_users,
                                                                     n_start, n_end):
    all_pos_df = pd.read_csv(user_item_file)
...
def generateFMTrainingTestingWithNegSampleGithubWithPartition(user_item_file, train_ratio, n_Neg, n_users, n_start,
                                                              n_end):
    all_pos_df = pd.read_csv(user_item_file)
...
def convertItemtoIndex(i_index, i_useritem, o_file):
    ...
    with open(o_file, 'a+', encoding='utf-8') as fout:
        ...
        for index, row in user_item_df.iterrows():
            ...
            fout.writelines(
                str(uid) + ',' + str(item_index_df.loc[rid]['index']) + ',' + str(answer) + ',' + str(favorite) + '\n')
        ...
        with connection.cursor() as cursor:
            ...
            with open(o_file, 'a+', encoding='utf-8') as fout:
                ...
                for start_at in range(int(math.ceil(total_rec / 1000))):
                    ...
                    for item in result:
                        fout.writelines(
                            str(item['ownerid']) + ',' + str(item['parentid']) + ',' + str(item['tags']) + '\n')
    finally:
        connection.close()
        ...
        with connection.cursor() as cursor:
            ...
            with open(o_file, 'a+', encoding='utf-8') as fout:
                ...
                for start_at in range(int(math.ceil(total_rec / 5000))):
                    ...
                    for item in result:
                        fout.writelines(
                            str(item['ownerid']) + ',' + str(item['fork_from']) + ',' + str(item['tags']) + '\n')
    finally:
        connection.close()
        ...
        with connection.cursor() as cursor:
            ...
            with open(o_file, 'a+', encoding='utf-8') as fout:
                ...
                for start_at in range(int(math.ceil(total_rec / 1000))):
                    ...
                    for item in result:
                        fout.writelines(
                            str(item['watcherid']) + ',' + str(item['repoid']) + ',' + str(item['tags']) + '\n')
    finally:
        connection.close()
...
def generateTrainTestByUser(user_item_file, train_ratio, n_users):
    ...
    # TODO rand???
    test_set = set(randsample(user_set, n_test))
            ...
            for i in range(5000):  # n_items):
                for k in range(k_neighbors):
                    if int(results[1][i][k]) != i:
                        fout.writelines(
                            str(i + start_index) + ',' + str(results[1][i][k]) + ',' + str(1 - results[0][i][k]) + '\n')
